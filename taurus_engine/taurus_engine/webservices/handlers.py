...
from taurus_engine.webservices.responses import UnauthorizedResponse
...
class AuthenticatedBaseHandler(object):
  ...
  def compareAuthorization(userAPIKey):
    ...
    if taurus_engine.config.has_section("security"):
      try:
        validAPIKey = taurus_engine.config.get("security", "apikey")
