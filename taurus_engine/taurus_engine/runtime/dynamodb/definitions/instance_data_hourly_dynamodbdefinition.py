...
class InstanceDataHourlyDynamoDBDefinition(DynamoDBDefinition):
  ...
  def tableName(self):
    return ("taurus.instance_data_hourly%s" %
            taurus_engine.config.get("dynamodb", "table_name_suffix"))
  ...
  def tableCreateKwargs(self):
    return dict(
      ...
      throughput={
        "read": (
          taurus_engine.config.getint("dynamodb",
      ...
      global_indexes=[
        GlobalAllIndex(
          ...
          throughput={
            "read": taurus_engine.config.getint(
                "dynamodb",
                "instance_data_hourly_throughput_read"),
            "write": taurus_engine.config.getint(
                "dynamodb",
                "instance_data_hourly_throughput_write")
          }
        )
