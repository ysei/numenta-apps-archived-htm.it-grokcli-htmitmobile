...
class DynamoDBService(object):
  ...
  def __init__(self):
    self._modelResultsExchange = (
      taurus_engine.config.get("metric_streamer", "results_exchange_name"))
    self._nonMetricDataExchange = (
      taurus_engine.config.get("non_metric_data", "exchange_name"))
  ...
  def connectDynamoDB():
    ...
    region = taurus_engine.config.get("dynamodb", "aws_region")

    connectKwargs = {
      "aws_access_key_id": os.getenv("AWS_ACCESS_KEY_ID", "taurus"),
      "aws_secret_access_key": os.getenv("AWS_SECRET_ACCESS_KEY", "taurus"),
      "is_secure": taurus_engine.config.getboolean("dynamodb", "is_secure"),
    }

    host = taurus_engine.config.get("dynamodb", "host")
    port = taurus_engine.config.get("dynamodb", "port")
  ...
  def run(self):
    ...
    def _configChannel(amqpClient):
      amqpClient.requestQoS(
          prefetchCount=taurus_engine.config.getint("dynamodb",
                                                    "prefetch_count"))
