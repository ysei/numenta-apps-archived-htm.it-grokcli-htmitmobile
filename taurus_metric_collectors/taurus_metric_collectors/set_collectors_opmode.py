...
def main():
  ...
  for section in taurus_metric_collectors.config.sections():
    if taurus_metric_collectors.config.has_option(section, "opmode"):
      if not configWriter.has_section(section):
        configWriter.add_section(section)

      configWriter.set(section, "opmode", opmode)

      oldOpmode = taurus_metric_collectors.config.get(section, "opmode")
